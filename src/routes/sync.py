...
def auth_callback():
    ...
    try:
        ...
        if not code:
            return jsonify({'error': 'No se proporcionó código de autorización'}), 400
        ...
        # En este caso específico, devolvemos JSON ya que es un endpoint de API
        # pero si el frontend espera una redirección, podríamos redirigir a una página de error
        return jsonify({'error': str(e), 'mensaje': 'Error en el callback de autenticación'}), 500
